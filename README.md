...
| `npm run build:social-preview` | Regenerate the Open Graph social preview image when its source art or copy changes. |
...
The committed `images/social-preview.jpg` is intentionally not regenerated by the normal build because JPEG encoder output can differ across platforms. Run `npm run build:social-preview` only when changing the preview artwork or text.
